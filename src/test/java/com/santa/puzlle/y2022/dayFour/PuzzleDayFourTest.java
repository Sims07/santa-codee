...
import com.santa.puzlle.y2022.daythree.partTwo.PuzzleDayThreePartTwo;
import com.santa.puzlle.y2022.daythree.partTwo.RuckSacks;
...
class PuzzleDayFourTest {
  ...
  void givenOverlap_isOverlap_shouldReturnTrue(){
    final boolean overlapped = Pair.from(
        List.of(CleaningSection.from(2, 8), CleaningSection.from(3, 7))).isOverlapped();
  ...
  void givenOverlapReverse_isOverlap_shouldReturnTrue(){
    final boolean overlapped = Pair.from(
        List.of(CleaningSection.from(3, 7), CleaningSection.from(2, 8))).isOverlapped();

    then(overlapped).isTrue();
  }

  @Test
  void givenNoOverlap_isOverlap_shouldReturnFalse(){
    final boolean overlapped = Pair.from(
        List.of(CleaningSection.from(2, 8), CleaningSection.from(3, 10))).isOverlapped();
  ...
  void givenPuzzle_getDuplicates_returnTheDuplicatedItem() throws IOException {
    Pairs pairs= new PuzzleDayFour().load("/Users/sc/Documents/projets/pocs/santa-codee/src/test/resources/2022/dayOne/dayFour/day4part1.txt");
